...
function PasswordWidget(divid,pwdname,extra_classes)
{
...
function _MakePWDWidget()
{
	...
	var fldname = pwdname.replace("_","[")+"]";

	code += "<input type='password' class='pwdfield "+ this.extra_classes +"' name='"+fldname+"' id='"+this.pwdfieldid+"' placeholder='Password' style='width:29%;'>";

	this.pwdtxtfield=pwdname+"_text";

	this.pwdtxtfieldid = this.pwdtxtfield+"_id";

	code += "<input type='text' class='pwdfield "+ this.extra_classes +"' name='"+this.pwdtxtfield+"' id='"+this.pwdtxtfieldid+"' placeholder='Password' style='display: none;width:29%;'>";

	this.pwdshowdiv = pwdname+"_showdiv";

	this.pwdshow_anch = pwdname + "_show_anch";

	code += "<div class='pwdopsdiv pwddiv2' id='"+this.pwdshowdiv+"'><a href='javascript:void(0)' id='"+this.pwdshow_anch+"'>"+this.txtShow+"</a></div>";

	this.pwdgendiv = pwdname+"_gendiv";

	this.pwdgenerate_anch = pwdname + "_gen_anch";

	code += "<div class='pwdopsdiv'id='"+this.pwdgendiv+"'><a href='javascript:void(0)' id='"+this.pwdgenerate_anch+"'>"+this.txtGenerate+"</a></div>";
...
function passwordStrength()
{
	...
	else if(score<5){ desc= this.txtStrong; }
	else if(score>=5){ desc= this.txtVStrong; }
